...
def update_game(instance,archive):
    ...
    'scores':scores,'player_order':instance.player_order})
    db.session.commit()
...
def pack_game(instance):
    ...
    for key in instance.players:
        ...
        letter_racks[key] = instance.players[key].letter_rack
        scores[key] = instance.players[key].score
    return GameArchive(instance.name, instance.board.board_matrix, instance.tilebag.letters, instance.board.dims, instance.max_rack_letters, player_list, scores, letter_racks, instance.player_order)
...
def unpack_game(archive):
    ...
    instance.add_players(num_players=len(archive.first().players), player_names=archive.first().players, scores=archive.first().scores, letter_racks=archive.first().letter_racks, player_order=archive.first().player_order)
    return instance
...
def cur_game(game_name):
    ...
    return render_template('board.html', name=game.name, height=game.board.dims[0]*s, width=game.board.dims[1]*s, square=s, board_matrix=game.board.board_matrix, player_list = [{'name':game.players[key].name, 'score':game.players[key].score, 'your_turn':game.players[key].name==game.player_order[0]} for key in game.players])
...
def player_view(game_name,player_name):
    ...
    if player_name != game.player_order[0]:
        #do something here to tell player it is not their turn
        pass
    ...
    return render_template('player.html', submit_form=player_form, swap_form=swap_form, letter_rack=game.players[player_name].letter_rack, num_letters=len(game.players[player_name].letter_rack), name=player_name, square=2*s, your_turn=player_name==game.player_order[0])
